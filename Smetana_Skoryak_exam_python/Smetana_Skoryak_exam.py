...
def data_with_map():
        lst_class = [terra.lands, lviv.streets, kyiv.streets, odessa.streets]
        for index, element in enumerate(lst_class, start=1):
            print(f'\n\t{index}  -  {element}')
...
def show_player_location(cube_num):
    return main_game_options[cube_num]
...
while True:
        ...
        if flage%2==0:            
            steps_1+=1
            flage+=1
            choice_data = input('Enter: ')
            num_cube = randint(1, 6)
            if main_game_options.index(show_player_location(num_cube)) < 5:
                print(f'\n\tBot on: {show_player_location(num_cube)}')
                data_with_map()
            else:
                print(f'\n\tBot on: {show_player_location(num_cube)}')
            ...
            num_cube = randint(1, 6)
            if main_game_options.index(show_player_location(num_cube)) < 5:
                print(f'\n\tPlayer on: {show_player_location(num_cube)}')
                data_with_map()
            else:
                print(f'\n\tPlayer on: {show_player_location(num_cube)}')
